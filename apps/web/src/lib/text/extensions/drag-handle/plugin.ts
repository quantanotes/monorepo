  ...
  function repositionDragHandle(dom: Element) {
    const virtualElement = {
      getBoundingClientRect: () => {
        ...
        const handleHeight = element.getBoundingClientRect().height;

        return {
          ...rect,
          top: rect.top + (rect.height - handleHeight) / 2,
          bottom: rect.top + (rect.height + handleHeight) / 2,
          height: handleHeight,
        };
      },
    };
      ...
      props: {
        handleDOMEvents: {
          ...
          mousemove(view, e) {
            ...
            if (outerNode?.type.name === 'settings') {
              hideHandle();
              return false;
            }
